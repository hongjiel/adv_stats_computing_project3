...
where $\Delta_{i,1}(t)$, $\Delta_{i,2}(t)$ and $\Delta_{i,3}(t)$ are the changes of latitude, longitude and wind speed between $t-6$ and $t$, $\boldsymbol{\beta}_{i} = (\beta_{0,i},\beta_{1,i},...,\beta_{4,i})^\top$ are the random coefficients, and $\mathbf{X}_i = (x_{i,1},\ldots,x_{i,6})^\top$ are covariates with fixed effects $\boldsymbol\gamma$. Here, $x_{i,1}$ is the calendar year of the $i$-th hurricane measured with respect to 1950 (with values 0 to 63), $x_{i,2}$ is the indicator variable of the month in active season (August-October) when the $i$-th hurricane started, and $x_{i,3},\ldots,x_{i,6}$ are indicator variables of the four types (ET, NR, SS, and TS) of the $i$-th hurricane when it began. We assume that the error term $\epsilon_{i}(t)\sim N(0,\sigma^2)$ and is independent across $t$.
...
Denote $n$ as the number of observed hurricanes and denote $\mathbf{B}= (\boldsymbol{\beta}_{1}^\top,...,\boldsymbol{\beta}_{n}^\top)^\top$. The joint prior distribution of $\Theta=(\mathbf{B}^\top,\boldsymbol{\mu}^\top, \boldsymbol\Sigma,\boldsymbol\gamma^\top,\sigma)$ is given by
\begin{align*}
...
Let $m_i$ denote the number of observations, excluding the first two observations and let $\mathbf{Y}_i=(Y_{i,1},\ldots Y_{i,m_i})^\top$ denote the wind speed data of the $i$-th hurricane, excluding the first two observations, where $Y_{i,j}=Y_i(6j+6)$. Denote $\mathbf{Y}=(\mathbf{Y}_1^\top,\mathbf{Y}_2^\top,\ldots,\mathbf{Y}_n^\top)^\top$ and $\mathbf Z_{i,j}=(1,Y_{i,j},\Delta_{i,1}(6j+6),\Delta_{i,2}(6j+6),\Delta_{i,3}(6j+6))^\top$.

Given that
$$Y_{i,j}\mid \mathbf{B}^\top,\boldsymbol\gamma^\top,\sigma\sim N(\mathbf Z_{i,j-1}^\top\boldsymbol\beta_i + \mathbf{X}_i^\top\boldsymbol\gamma,\sigma^2),\qquad\forall i=1,\ldots,n,\; j=1,\cdots,m_i$$
the likelihood function of parameters $\Theta=(\mathbf{B}^\top,\boldsymbol{\mu}^\top, \boldsymbol\Sigma,\boldsymbol\gamma^\top,\sigma)$ is given by
$$\begin{aligned}
L_{\mathbf{Y}}(\Theta)=&\ \prod_{i=1}^nL_{\mathbf{Y}_i}(\mathbf{B}^\top,\boldsymbol{\mu}^\top,\boldsymbol{\Sigma},\boldsymbol\gamma^\top,\sigma)\\
...
The joint posterior distribution of $\Theta=(\mathbf{B}^\top,\boldsymbol{\mu}^\top, \boldsymbol\Sigma,\boldsymbol\gamma^\top,\sigma)$ is given by
\begin{align}
\pi(\Theta\mid \mathbf{Y}^\top)
\propto &\ L_{\mathbf{Y}}(\Theta)\pi(\Theta)\nonumber\\
...
We aim to sample each parameter from the joint posterior distribution $\pi(\Theta\mid \mathbf{Y}^\top)$, but but direct sampling is impractical. Fortunately, the proposed Bayesian model includes several conditionally conjugate priors, which facilitate posterior computations using a hybrid Metropolis-Hastings/Gibbs algorithm. Given that $\pi(\Theta\mid \mathbf{Y}^\top)$ takes the form (\ref{post}), the conditional posterior distribution for each parameter is as follows:

1. $\boldsymbol\beta_i\mid \boldsymbol{\mu}^\top,\boldsymbol{\Sigma},\boldsymbol\gamma^\top,\sigma,\mathbf{Y}^\top$ follows a multivariate normal distribution with mean vector
...
$$\pi(\sigma\mid \mathbf{B}^\top,\boldsymbol{\mu}^\top,\boldsymbol{\Sigma},\boldsymbol\gamma^\top,\mathbf{Y}^\top) \propto\ I(\sigma>0)\frac{\sigma^{-\sum_{i=1}^n m_i}}{1+(\sigma/10)^2}\exp\left(-\frac{1}{2\sigma^2}\sum_{i=1}^n\sum_{j=1}^{m_i}(Y_{i,j}-\mathbf Z_{i,j-1}^\top\boldsymbol\beta_i-\mathbf{X}_i^\top\boldsymbol\gamma)^2\right).$$
...
Sample $\boldsymbol{\beta}_i^{(k)}$ from $\pi(\boldsymbol\beta_i\mid \boldsymbol{\mu}^{(k-1)},\boldsymbol{\Sigma}^{(k-1)},\boldsymbol\gamma^{(k-1)},\sigma^{(k-1)},\mathbf{Y}^\top)$  
Sample $\boldsymbol{\mu}^{(k)}$ from $\pi(\boldsymbol{\mu}\mid\mathbf B^{(k)}, \boldsymbol{\Sigma}^{(k-1)},\boldsymbol{\gamma}^{(k-1)}, \sigma^{(k-1)}, \mathbf{Y}^\top)$  
...
The specific value we choose can be seen at table\ref{table:initial}.

## Parameter convergence diagnostic

Through trace plots, it can be seen that the convergence speed of each parameter is very fast, and after 1000 iterations, all parameters except the intercept of $\mu_0$ have converged with fluctuations, while the others have reached convergence.They are shown in the figure\ref{trace plot}.

It's worth noting that, through continuous effort and data adjustment, while $\mu_0$ did not strictly converge, it has stabilized within a reasonable range, and its convergence has significantly improved compared to before the adjustments were made. For instance, we scaled the variable season(represent year), which stabilized the range of $\mu_0$ and improved the convergence of the it.

### Autocorrelation plot

We have plotted an autocorrelation plot and found that after selecting a burn-in of 5000, the correlation of all parameters in the samples decreases rapidly and eventually stabilizes near zero. This indicates that the chosen burn-in period is very good and the samples used for inference can be assumed to be uncorrelated.

It can be seen from the figure \ref{auto plot} that the autocorrelation of each parameter decreases rapidly and eventually stabilizes near zero.

### Histogram plot
By examining the histograms, we primarily focused on the numerical distribution of the important parameters, namely $\boldsymbol{\mu}$ and $\boldsymbol{\Sigma}$. We found that their distributions were normal-like and had no anomalies. Our estimated values fell in the central region of the histograms, indicating a good fit to the normal distribution. They are shown in the figure \ref{historam plot}.Furthermore, we creat CI for $\boldsymbol{\mu}$ at table \ref{table:mu Ci}.

## Hyperparameter Value of choosing
\textbf{Hyperparameter Value}
\begin{itemize}
 \item Search window $a=0.1$(chosen such that acceptance rate of proposed sample is around 0.4) 
...
Given the estimates of parameters and known variables, the predicted wind speed of the $i$-th hurricane at $t+6$ time stamp, denoted as $\hat{Y}_i(t+6)$ could be computed as:
$$\hat{Y}_i(t+6) = \hat{\beta}_{0,i} + \hat{\beta}_{1,i}Y_i(t) + \hat{\beta}_{2,i}\Delta_{i,1}(t) + \hat{\beta}_{3,i}\Delta_{i,2}(t) + \hat{\beta}_{4,i}\Delta_{i,3}(t) + \mathbf{X_i}\hat{\gamma}$$
where $\bm{\hat{\beta_i}} = (\hat{\beta}_{0,i}, \hat{\beta}_{1,i}, \hat{\beta}_{2,i}, \hat{\beta}_{3,i}, \hat{\beta}_{4,i})$ are the estimated random coefficients, $\hat{\gamma}$ is the estimated fixed effect of $\mathbf{X_i}$, and other variables are the same as explained and denoted in Chapter 2.\par

Based on the burn-in number $5000$ as concluded in MCMC parameter convergence diagnostic section, we calculated the parameters to be used to make prediction, and predict the wind speeds of each hurricane at each time stamp, except for the first two time stamps. We computed the RMSE and R-squared of each hurricane, as partly shown in the table \ref{table:summary}. It can be observed that different hurricanes perform variously on RMSE and R-squared. Generally the RMSE for selected hurricanes is around $3$ to $8$, and the R-squared is within $0.7$ to $1.0$. However, there is also extremely low R-squared($0.30$ as shown) and high RMSE($9.64$ as shown).\par
...
For the parameter $\gamma_{year}$, at first it had a really bad performance on convergence because we used the origin data of year i.e. 1950-2013. After presentation, thanks to the advice given by Professor Ying Wei, we tried to scale the data to 0-63 since one-year change had little effect on the original year. The convergence then seems pretty good after the adjustment. This may give us a lesson that we should try to scale some of the data if any parameter does not converge through MCMC.

## Prediction latency of wind speed change

It can be observed from the plots that there exists a prediction latency towards the changing of wind speed  at different time points. We suspect that because the model is based on the speed trend from $t-6$ to $t$, thus it tends to continue this trend simulation from $t$ to $t+6$, while $t+6$ may be a complete different situation. Besides, the measurement of wind speed is in unit of $5$ Knot and not that precise, which may also be a reason for inadequate accurate prediction. Further investigation may be needed concerning time series models explanation.
...
Jiajun Tao worked on posterior summaries, credible intervals, and diagnostics of $\gamma$, and was also responsible for the corresponding part of the presentation and the report. Here I would like to express my appreciation to my teammates. I had a great experience in working on this project. The hardest part should be the derivation and the algorithm of MCMC, they really did a great job. Nice to work with those guys!

Xicheng Xie worked on EDA, model building for MCMC starting value selection, and MCMC algorithm running. He was also responsible for the corresponding part of the presentation, slide-making, and report. Thanks to all the teammates for their collaboration and contribution to this project. The posterior distribution calculation and MCMC building are tough but we all gained a lot from this project. 

Shaohan Chen worked on revising MCMC R functions, predicting wind speed for each time point of different hurricanes, and evaluating the estimation performance. Meanwhile, he was also responsible for the corresponding part of the presentation slide and report. Many thanks to my nice teammates and their excellent work, especially the impressive math calculations and MCMC implementation. Special thanks to Xicheng, who helped deliver the "task 4" presentation for me, when I was still in recovery of my left foot wound and unable to attend our presentation in person. Lastly, I would like to give the best credit again to our team again for everybody's great contribution and devotion to the project. Looking forward to our next cooperation.

Zijian Xu worked on analysis of the MCMC algorithm, including reviewing the algorithm, writing functions to check for errors, visualizing results, conducting diagnostic analysis, and evaluating and improving starting values. Additionally, I wrote the front part of section 3 of the report, which focused on the MCMC algorithm, and contributed to delivering the presentation for task 2.I would like to express my utmost gratitude to my team members who were always available and willing to contribute their efforts to the project.I would like to say that my teammates are humble. In reality, a lot of our work cannot be quantified, and in my opinion, the amount of work we have accomplished is tremendous. For example, writing the report entirely in LaTeX formulas, ensuring the report was well-organized and complete, and continuously formatting and adjusting it. Moreover, each team member had to adjust their sections after every change in the project's direction, which shows that every member was indispensable to the project's success. The project is a result of everyone's hard work and dedication.
...
\begin{table}[h]
    ...
    \begin{tabular}{|c|c|}
        ...
        Parameter & Value \\
        \hline
        $\boldsymbol{\mu}^\top$ & $(2.91, 0.94, -0.03, -0.24, 0.47)$ \\
        $\boldsymbol{\gamma}^\top$ & $(-0.01, 0.35, 0.28, 0.37, 0.12, 0.08)$ \\
        $\boldsymbol{\Sigma}$ & $\begin{pmatrix}
           0.390 & -0.011 & 0.021 & 0.140 & 0.031 \\
          -0.011 & 0.001 & -0.002 & -0.005 & 0.000 \\
           0.021 & -0.002 & 0.034 & 0.026 & -0.019 \\
           0.140 & -0.005 & 0.026 & 0.074 & 0.003 \\
           0.031 & 0.000 & -0.019 & 0.003 & 0.018 \\
        \end{pmatrix}$ \\
        $\sigma$ & 5.27 \\
        \hline
    \end{tabular}
...
\begin{table}[h]
\centering
\caption{Summary statistics for $\boldsymbol\mu$}
\label{table:mu Ci}
\begin{tabular}{lccc}
\hline
Parameter & mu\_mean\ &   lower bound &   upper bound \\ \hline
$\mu_0$     & 4.5091968     & 3.8894989     & 5.0617140     \\
$\mu_1$     & 0.9040681     & 0.8397420     & 0.9679912     \\
$\mu_2$     & -0.0488009    & -0.2472724    & 0.1533209     \\
$\mu_3$     & -0.4617278    & -0.5988835    & -0.3236038    \\
$\mu_4$     & 0.4714829     & 0.3978996     & 0.5459744     \\ \hline
\end{tabular}
\end{table}
...
\begin{figure}[ht] 
  \centering 
  \subfigure[mu]{ 
    \includegraphics[width=3.0in, height = 1.8in]{parameters_burnin/Mu_parameters_time_series_plot.png} 
  } 
  \subfigure[Sigma matrix]{ 
    \includegraphics[width=3.0in, height = 1.8in]{parameters_burnin/Sigma_parameters_time_series_plot.png}
  } \\
  \subfigure[gamma]{ 
    \includegraphics[width=3.0in, height = 1.8in]{parameters_burnin/Gamma_parameters_time_series_plot.png} 
  } 
  \subfigure[sigma]{ 
    \includegraphics[width=3.0in, height = 1.8in]{parameters_burnin/igma_parameters_time_series_plot.png} 
  } 
  \caption{trace plot}
  \label{trace plot}
\end{figure}
...
\begin{figure}[ht] 
  \centering 
  \subfigure[mu]{ 
    \includegraphics[width=3.0in, height = 1.8in]{parameters_burnin/Mu_Parameters_ACF_Plot.png} 
  } 
  \subfigure[Sigma matrix]{ 
    \includegraphics[width=3.0in, height = 1.8in]{parameters_burnin/Sigma_Parameters_ACF_Plot.png} 
  } \\
  \subfigure[gamma]{ 
    \includegraphics[width=3.0in, height = 1.8in]{parameters_burnin/Gamma_Parameters_ACF_Plot.png} 
  } 
  \subfigure[sigma]{ 
    \includegraphics[width=3.0in, height = 1.8in]{parameters_burnin/igma_Parameters_ACF_Plot.png} 
  }
  \caption{auto plot}
  \label{auto plot} 
\end{figure}
...
\begin{figure}[ht] 
  \centering 
  \subfigure[mu]{ 
    \includegraphics[width=3.0in, height = 2.0in]{parameters_burnin/Mu_Parameters_Histogram_Plot.png} 
  } 
  \subfigure[Sigma matrix]{ 
    \includegraphics[width=3.0in, height = 2.0in]{parameters_burnin/Sigma_Parameters_Histogram_Plot.png} 
  } 
  \caption{historam plot}  
  \label{historam plot} 
\end{figure}
...
1. $\mathbf{B}$:
$$\begin{aligned}
&\ \pi(\boldsymbol\beta_i\mid \boldsymbol{\mu}^\top,\boldsymbol{\Sigma},\boldsymbol\gamma^\top,\sigma,\mathbf{Y}^\top)\\
...
Thus we have
$$\begin{aligned}\\
&\ \boldsymbol\beta_i\mid \boldsymbol{\mu}^\top,\boldsymbol{\Sigma},\boldsymbol\gamma^\top,\sigma,\mathbf{Y}^\top\\
...
2. $\boldsymbol{\mu}$:
$$\begin{aligned}
&\ \pi(\boldsymbol{\mu}\mid \mathbf{B}^\top,\boldsymbol{\Sigma},\boldsymbol\gamma^\top,\sigma,\mathbf{Y}^\top)\\
...
\end{aligned}$$
Thus we have
$$\boldsymbol{\mu}\mid \mathbf{B}^\top,\boldsymbol{\Sigma},\boldsymbol\gamma^\top,\sigma,\mathbf{Y}^\top\sim N\left(\boldsymbol V(n\boldsymbol V+\boldsymbol\Sigma)^{-1}\left(\sum_{i=1}^n\boldsymbol\beta_i\right), \boldsymbol V(n\boldsymbol V+\boldsymbol\Sigma)^{-1}\right).$$
3. $\boldsymbol{\Sigma}$:
$$\begin{aligned}
\pi(\boldsymbol{\Sigma}\mid\mathbf{B}^\top,\boldsymbol{\mu}^\top,\boldsymbol\gamma^\top,\sigma,\mathbf{Y}^\top)
...
\end{aligned}$$
Thus we have
$$\boldsymbol{\Sigma}\mid \mathbf{B}^\top,\boldsymbol{\mu}^\top,\boldsymbol\gamma^\top,\sigma,\mathbf{Y}^\top\sim \mathcal{W}^{-1}\left(\boldsymbol{S}+\sum_{i=1}^n(\boldsymbol\beta_i-\boldsymbol\mu)(\boldsymbol\beta_i-\boldsymbol\mu)^\top, n+\nu\right).$$
4. $\boldsymbol\gamma$:
$$\begin{aligned}
&\ \pi(\boldsymbol\gamma\mid \mathbf{B}^\top,\boldsymbol{\mu}^\top,\boldsymbol{\Sigma},\sigma,\mathbf{Y}^\top)\\
...
Thus we have
$$\begin{aligned}
&\ \boldsymbol\gamma\mid \mathbf{B}^\top,\boldsymbol{\mu}^\top,\boldsymbol{\Sigma},\sigma,\mathbf{Y}^\top\\
