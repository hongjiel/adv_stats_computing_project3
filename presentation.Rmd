...
## Task 2 - Posterior Distribution (part1)
**Conditional Posterior for each parameter**

\fontsize{5pt}{3pt}\selectfont
...
\fontsize{5pt}{3pt}\selectfont
$$\begin{aligned}
&\ \boldsymbol\gamma\mid (\mathbf{B}^\top,\boldsymbol{\mu}^\top,\boldsymbol{\Sigma},\sigma,\boldsymbol{Y}^\top)\sim N\left(\left(400\boldsymbol I+\frac{1}{\sigma^2}\sum_{i=1}^nm_i\mathbf{X}_i\mathbf{X}_i^\top\right)^{-1}\left(-\frac{1}{\sigma^2}\sum_{i=1}^n\sum_{j=1}^{m_i}(\mathbf Z_{i,j-1}^\top\boldsymbol\beta_i-Y_{i,j})\mathbf{X}_i\right),\left(400\boldsymbol I+\frac{1}{\sigma^2}\sum_{i=1}^nm_i\mathbf{X}_i\mathbf{X}_i^\top\right)^{-1}\right).
\end{aligned}$$
\fontsize{5pt}{3pt}\selectfont
...
\begin{algorithm}[H]
 ...
 \begin{algorithmic}
   ...
   \For{$i=1$ to $iternumber$}
     ...
     \State Gibbs sampling for $\Sigma$\par $\boldsymbol{\Sigma}^{(k)} \sim Wishart(\boldsymbol{\Sigma}|\mathbf{Y}, \boldsymbol{\beta}^{(k)}, \boldsymbol{\mu}^{(k)}, \sigma^{(k-1)}, \gamma^{(k-1)})$
     \State Gibbs sampling for $\boldsymbol{\gamma}$\par $\boldsymbol{\gamma}^{(k)} \sim p(\boldsymbol{\Sigma}|\mathbf{Y}, \boldsymbol{\beta}^{(k)}, \boldsymbol{\mu}^{(k)}, , \Sigma^{(k)},\sigma^{(k-1)})$
     \State Metropolis-Hastings for $\sigma$
      \State Propose a conditional distribution of new value $\sigma^*$ from a Uniform distribution, a=0.1.
      \State Compute the acceptance ratio\par
      $\gamma = \frac{p(\gamma^{(k)}|\mathbf{Y}, \boldsymbol{\beta}^{(k)}, \boldsymbol{\mu}^{(k)}, \boldsymbol{\Sigma}^{(k)},\sigma^{(k)})q(\gamma^{(k-1)}|\gamma^{(k)})}{p(\gamma^{(k-1)}|\mathbf{Y}, \boldsymbol{\beta}^{(k)}, \boldsymbol{\mu}^{(k)}, \boldsymbol{\Sigma}^{(k)}, \sigma^{(k)})q(\gamma^{(k)}|\gamma^{(k-1)})}$\par
      $\alpha=\min(1,\gamma)$\par
      \State Generate a random number $u$ from a uniform distribution between 0 and 1.
      \State If $u \leq \alpha$, set $\sigma^{(k)} = \sigma^*$, otherwise set $\sigma^{(k)} = \sigma^{(k-1)}$.(The Final acceptance rate is 0.418)
    \EndFor
  \State 
  \end{algorithmic}
\end{algorithm}
...
\tiny
 \begin{columns}
  \begin{column}{0.4\textwidth}
    ...
    \item$\Sigma$: This can be obtained through the VarCorr(lmm) function which returns the covariance matrix of the random effects in the model $\Sigma^{^{(0)}}$.
    \end{itemize}
  \end{column}
...
##  Task 2 - Results Presentation(Parameters Estimation of $mu$ and $Sigma$)
\centering
\tiny
\textbf{$\hat{\mu}$}
\[
...
##  Task 2 - Results Presentation(CI of $mu$)
```{r warning=FALSE, echo=FALSE}
data <- data.frame(
...
- Conclusion: 
  ...
  1. There are no seasonal difference (Active v.s inactive) in hurricane wind speeds.
