 ...
 \begin{algorithmic}
   ...
   \For{$i=1$ to $iternumber$}
     ...
     \State Metropolis-Hastings for $\sigma$
      ...
      $\lambda = \frac{p(\sigma^{(k)}|\mathbf{Y}, \boldsymbol{\beta}^{(k)}, \boldsymbol{\mu}^{(k)}, \boldsymbol{\Sigma}^{(k)},\gamma^{(k)})q(\sigma^{(k-1)}|\sigma^{(k)})}{p(\sigma^{(k-1)}|\mathbf{Y}, \boldsymbol{\beta}^{(k)}, \boldsymbol{\mu}^{(k)}, \boldsymbol{\Sigma}^{(k)}, \gamma^{(k)})q(\sigma^{(k)}|\sigma^{(k-1)})}$\par
...
\begin{center}
search window $a=0.1$, burnin $=8000$, accept ratio $\alpha=0.1$
\end{center}
