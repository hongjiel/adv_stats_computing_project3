...
Let $n$ denote the number of hurricanes in the training data. \textcolor{blue}{(we need to partition all hurricanes into training \& test.)}
...
Let $m_i$ denote the number of observations for the $i$-th hurricane, excluding the first \textcolor{red}{and second} observation. We denote the wind speed data for the $i$-th hurricane (excluding the first \textcolor{red}{and second} observation) as $\boldsymbol{Y}_i=(Y_{i,1},\ldots Y_{i,m_i})^\top$, where $Y_{i,k}$ represents the wind speed at time $t_{0,i}+6k$, and $t_{0,i}$ is the time of the \textcolor{red}{second} observation for the $i$-th hurricane. We denote the wind speed data for all hurricanes as $\boldsymbol{Y}=(\boldsymbol{Y}_1^\top,\boldsymbol{Y}_2^\top,\ldots,\boldsymbol{Y}_n^\top)^\top$. We denote $\Delta_{i,k,1}$, $\Delta_{i,k,2}$ and $\Delta_{i,k,3}$ as $\Delta_{i,1}(t_{0,i}+6k)$, $\Delta_{i,2}(t_{0,i}+6k)$ and $\Delta_{i,3}(t_{0,i}+6k)$, and denote $\mathbf Z_{i,k}=(1,Y_{i,k},\Delta_{i,k,1},\Delta_{i,k,2},\Delta_{i,k,3})^\top$. (\textcolor{blue}{$\mathbf Z_{i,k}$ contains $Y_{i,k}$, the previous observation of wind speed})
