...
First import parameters from task 1 & 2.
```{r}
# load parameters
...
Prediction of wind speed for each hurricane.
```{r}
...
Windspeed_Predict = function(index_hurricane, index_time){
  predict_speed = Z[[index_hurricane]][index_time,] %*% para_beta[index_hurricane:(index_hurricane + 4)] + (X %*% t(para_gamma))[index_hurricane, ]
  return(predict_speed)
}

# functional test
Windspeed_Predict(2, 6)
```
